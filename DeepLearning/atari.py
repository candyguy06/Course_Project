...
class Atari:
    def __init__(self):
        ...
        self._main_loop(game_mode, self._game_model(game_mode, game_name, env.action_space.n), env, render, total_step_limit, total_run_limit, clip)

    def _main_loop(self, game_mode, game_model, env, render, total_step_limit, total_run_limit, clip):
        run = 0
        total_step = 0	
            ...
            while True:
                ...
                while True:
                    ...
                    if clip:
                        np.sign(reward)
                    score += reward
                    game_model._remember(current_state, action, reward, next_state, terminal)
                    current_state = next_state

                    game_model._step_update(total_step)

                    if terminal:
                        game_model._save_run(score, step, run)
                        break
