...
def network(step_input, state):
    ...
    lstm_cell = tf.nn.rnn_cell.LSTMCell(num_units=NN,name='basic_lstm_cell')
...
with graph.as_default():
    ...
    # placeholders
    input_data = tf.placeholder(tf.float32, [None, X]) # [row,column] dim
    ...
    #initial_nn_state = tf.nn.rnn_cell.BasicLSTMCell(NN).zero_state(1, tf.float32) # original
    initial_nn_state = tf.nn.rnn_cell.LSTMCell(num_units=NN,name='basic_lstm_cell').zero_state(1, tf.float32)
    #print("NN :", initial_nn_state)
    ...
    def step_op(time, memory_state, controller_state, inputs, outputs):
        ...
        #print("step_input :", step_input.get_shape())
        
        M, u, p, L, wr, ww, r = memory_state
        #print("reshape : ", tf.reshape(r, [-1]))

        with tf.variable_scope('controller'):
            Xt = tf.concat([step_input, tf.reshape(r, [-1])], 0)
            nn_output, nn_state = network(Xt, controller_state)
            std = lambda input_size: np.min(0.01, np.sqrt(2. / input_size).astype(np.int32))
    ...
    # packed_output = final_outputs.pack() # original
    packed_output = final_outputs.stack()

    loss = tf.reduce_mean(
        #loss_weights * tf.nn.softmax_cross_entropy_with_logits(labels=packed_output, logits=target_output) # original
        loss_weights * tf.nn.softmax_cross_entropy_with_logits_v2(labels=target_output, logits=packed_output))
...
# END: Computational Graph
    ...
    with tf.Session(graph=graph) as session:
        ...
        #session.run(tf.initialize_all_variables()) # original
